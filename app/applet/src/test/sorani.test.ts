...
import { parseChatRequest, parseAssessmentRequest, parseReportRequest, parseStudyVisionRequest } from "../server/ai/AiContracts.ts";
...
test("Sorani UX: missing study question", () => {
  // Wait, study question is actually also using parseChatRequest maybe? Or is it parseStudyVisionRequest?
  // Let's test vision request missing image first
  assert.throws(() => {
    parseStudyVisionRequest({});
  }, /وێنەکە دیاری نەکراوە یان بە فۆرماتی دروست نییە\./);
});
