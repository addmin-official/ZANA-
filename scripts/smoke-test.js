...
async function expectHealth() {
  const response = await fetchWithRetry("/api/health", { headers: { Origin: corsOrigin } });
  const payload = await response.json();
  assert(response.status === 200, `/api/health returned ${response.status}`);
  assert(payload.status === "ok" && payload.service === "zana-api-worker", "/api/health returned an invalid payload");
  assert(response.headers.get("access-control-allow-origin") === corsOrigin, "/api/health returned an invalid CORS origin");
  console.log("PASS API health and CORS");
}

async function expectApiFallbackIsJson() {
  const response = await fetchWithRetry("/api/__zana_smoke_not_found__", { headers: { Origin: corsOrigin } });
  const body = await response.text();
  assert(response.status === 404, `unknown API route returned ${response.status}; expected 404`);
  assert((response.headers.get("content-type") || "").includes("application/json"), "unknown API route did not return JSON");
  JSON.parse(body);
