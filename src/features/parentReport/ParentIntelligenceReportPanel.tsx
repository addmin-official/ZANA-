...
export function ParentIntelligenceReportPanel({
  ...
  // Safe helper to build deterministic internal code from profile info and timestamp
  const reportCode = `PR-ZANA-${snapshot.studentName.slice(0, 3).toUpperCase()}-${new Date(snapshot.generatedAt).getTime().toString().slice(-6)}`;
